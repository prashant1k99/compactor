...
func GenerateDOT(root Node) string {
	dot := "digraph G {\n"
	dot += generateDOTNode(root, "root")
	dot += "}\n"
	return dot
}

func generateDOTNode(node Node, id string) string {
	if node.IsLeaf() {
		return fmt.Sprintf("%s [label=\"%c\"];\n", id, node.Char())
	}

	dot := fmt.Sprintf("%s [label=\"%d\"];\n", id, node.Frequency())
	children := node.Child()
	for i, child := range children {
		childID := fmt.Sprintf("%s_%d", id, i)
		dot += fmt.Sprintf("%s -> %s;\n", id, childID)
		dot += generateDOTNode(child, childID)
	}
	return dot
}
